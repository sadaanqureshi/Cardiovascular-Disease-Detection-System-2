...
import { 
  ...
  LogOut, 
  ChevronUp 
} from 'lucide-react'; // LogOut aur ChevronUp icons add kiye
...
const Sidebar: React.FC<SidebarProps> = ({ role = 'admin', isOpen = false, onClose }) => {
  ...
  const [isUserMenuOpen, setIsUserMenuOpen] = useState(false); // Menu toggle state
  ...
  // Logout Handler
  const handleLogout = () => {
    // Yahan tum token clear karne ka logic laga sakte ho (e.g., localStorage.removeItem('token'))
        ...
        <div className="p-4 border-t border-slate-50 relative">
           ...
           >
              ...
              <div className="overflow-hidden text-left flex-1">
                  <p className="text-sm font-bold text-slate-700 capitalize truncate">{role} User</p>
